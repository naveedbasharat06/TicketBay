...
import { ticketState } from "@/constants";
...
const TicketOrdering: FC<props> = React.memo(
  ({ ticketPricing, moveNext, setMoveNext }) => {
    const isSmallScreen = useMedia("(max-width: 600px)");
    const isMediumScreen = useMedia(
      "(min-width: 601px) and (max-width: 1023px)"
    );
    const regular = ticketPricing?.regular;
    const [ticketDetails, setTicketDetails] = useState<any>(ticketState);
    ...
    useEffect(() => {
      if (moveNext) {
        if (total > 0) {
          ticketStore.addticket(ticketDetails);
    ...
    return (
      <div className="lg:grid grid-cols-5 w-full gap-5 mt-10">
        ...
        <div
          className={`col-span-2 bg-[#E3F5FF] rounded-[4px] p-5 ${
            isMediumScreen ? "mt-5" : isSmallScreen && "mt-5"
          }`}
        >
          <div>
          ...
          <div className="w-full p-2 bg-[#ffff] rounded-[4px]">
            ...
            <div className="mt-5">
              <button
                className="bg-[#FD2F09] w-full h-[48px] rounded-[4px] text-[#ffff]"
                onClick={() => {
                  setMoveNext(true);
                }}
              >
